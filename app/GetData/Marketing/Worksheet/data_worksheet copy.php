...
class data_worksheet{
    public function data_index($data)
    {
        ...
        // dd($index);
        $test = collect($index)->where('no_po', null);
        // dd($test);
        return $index;
    }
    ...
    public function data_worksheet($data)
    {
        ...
        foreach ($items as $key => $value) {
                ...
                $index[] = [
                    ...
                    'article' => $value->general_identity->article,
    ...
    public function copy_worksheet()
    {
        ...
        foreach ($worksheet as $key => $value) {
            $copy_worksheet[] = [
                'master_id' => $value->id,
                'no_po' => $value->general_identity->no_po,
                'style_name' => $value->general_identity->style_name,
            ]; 
        }

        return $copy_worksheet;
        // dd($copy_worksheet);
        // if ($data_worksheet != null || $data_po_tambahan != null ) {
        //     $result= $data_worksheet.','.$data_po_tambahan;
        //     $hasil = explode(',', $result);
        //     foreach ($hasil as $key => $value) {
        //         $rekap_order = RekapOrder::where('id',$value)->first();
        //         if ($rekap_order != null) {
        //             $copy_worksheet[] = [
        //                 "master_id" => $rekap_order->id,
        //                 "no_po" => $rekap_order->no_po,
        //                 // "style" => $rekap_order->general_identity->style_code,
        //             ];
        //         }
        //     }
        // }
