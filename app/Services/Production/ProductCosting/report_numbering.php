...
class report_numbering{
    public function numbering_index($data_form)
    {
        $data = [];
        foreach ($data_form as $key => $value) {
            ...
            foreach ($wo as $key2 => $value2) {
                ...
                foreach ($color as $key3 => $value3) {
                    foreach ($value->proses_numbering_user as $key4 => $value4) {
                        // Inisialisasi Awal 
                        ...
                        $banyak_user = collect($value->proses_numbering_user)->count('id');
