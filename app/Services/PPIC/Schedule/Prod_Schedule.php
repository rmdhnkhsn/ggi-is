...
class Prod_Schedule{
    ...
    public function adjust_schedule($request) 
    {
        ...
        // disabled karena di lapangannya di block
    ...
    public function calc_finish_wo($wo_id,$wo_no,$qt_order,$qt_adjsupply,$complete,$target,$lc1,$lc2,$lc3,$start_dt,$line_id) //hour balance
    {
        ...
        //fitur baru bu nunung, karena shipdate ordernya jauh, dan blm ada
        //order baru lg
        $cek_wo=WorkOrder::find($wo_id);
        if ($cek_wo) {
            if ($cek_wo->plot_setting=='PICK_DATE') {
                $start_dt=$cek_wo->start_date;
            }
        }
        ...
        while ($qty_order>0) {
            ...
            if ($fday==1) {
                $arr_output_before=$this->get_output_before($fday,$qty_order,$prodline->sub,$line_id,$prodline->line,$wo_id,$wo_no,$start_dt,$is_tgl_output_original);
            }
            //end cari
            ...
            if ($prodline->line_refactor($prodline->sub)) {
                $mon=Monitoring::where('wo',$wo_no)->where('tanggal',$start_dt)->get();
                foreach ($mon as $key => $value) {
                    // $value->line=preg_replace("/[^0-9]/", "", $value->line);
                    if (strlen($value->line>=2)) {
                        $prefix=substr($value->line,0,1);
                        if ($prefix=="L") {
                            $line_t=substr($value->line,1);
                            $line_t=preg_replace("/[^0-9]/", "", $line_t);
                            $value->line=$prefix.$line_t;
                        } else {
                            $value->line=preg_replace("/[^0-9]/", "", $value->line);
                        }
                    } else {
            ...
            if ($mon) {
                ...
                if ($mon['jam_kerja']!==null) {
                    ...
                    $get_jamker=$this->get_jamker($mon);
                    $jk=$get_jamker['jk'];
                    $b_hour=$get_jamker['b_hour'];
                }
        ...
        $this->get_output_after($arr_target,$prodline->sub,$wo_id,$wo_no,$line_id,$prodline->line,$start_dt,$is_tgl_output_original);
        //end cari

        if ($arr_target==null) {
            //adj supply = qty order jadi gak ada output, tetap harus kembalikan array agar tidak error
            $arr_target[0]=['workorder_id'=>$wo_id,'wo_no'=>$wo_no,'production_line_id'=>$line_id,'target_date'=>$start_dt,
                            'h1'=>0,'h2'=>0,'h3'=>0,'h4'=>0,'h5'=>0,'h6'=>0,'h7'=>0,'h8'=>0,'h9'=>0,'h10'=>0,'h11'=>0,'h12'=>0,'h13'=>0,'h14'=>0,
                            'total_hour'=>0,'total_actual'=>0,'balance_hour'=>0];
        } 

        return $arr_target;
    }
    public function get_output_before($fday,&$qty_order,$sub,$line_id,$line,$wo_id,$wo_no,$start_dt,$is_tgl_output_original) 
    {
        //cari output sebelum tanggal plot
        $prdln=new ProductionLine();
        $arr_output_before=[];
        if ($fday==1) {
            $mon=null;
            // $prodline->line_refactor($prodline->sub)
            if ($prdln->line_refactor($sub)) {
                $mon=Monitoring::where('wo',$wo_no)->where('tanggal','<',$start_dt)->get();
                foreach ($mon as $key => $value) {
                    // $value->line=preg_replace("/[^0-9]/", "", $value->line);
                    if (strlen($value->line>=2)) {
                ...
                $mon=$mon->where('line',$line);
            } else {
                $mon=Monitoring::where('wo',$wo_no)->where('tanggal','<',$start_dt)->where('line',$line)->get();
            }

            // cari wo split
            $wo_before=WorkOrder::where('id','<>',$wo_id)->where('wo_no',$wo_no)->where('production_line_id',$line_id)->where('start_date','<',$start_dt)->orderBy('start_date','asc')->get()->map(function ($data) {
                return [
                    'id'=>$data->id,
                    'wo'=>$data->wo_no,
                    'tgl_start'=>$data->start_date,
                    'qty_order'=>$data->qty_order,
                    'output'=>0,
                    'balance'=>$data->qty_order
                ];
            });
            $wo_before=$wo_before->toArray();
            $is_habis=false;
            foreach ($mon as $value) {
                $wo_before_collect=collect($wo_before);
                if ($wo_before_collect->sum('balance')<=0) {
                    $is_habis=true;
                }
                $total_qty_hour=$value->total_outpot;
                foreach ($wo_before as $k=>$v1) {
                    if ($v1['balance']>0) {
                        $wo_before[$k]['balance']-=$total_qty_hour;
                        $wo_before[$k]['output']+=$total_qty_hour;
                        break;
                    }
                }
        ...
        return $arr_output_before;
        //end cari
    }
    public function get_output_after(&$arr_target,$sub,$wo_id,$wo_no,$line_id,$line,$start_dt,$is_tgl_output_original)
    {
        //cari output setelah tanggal plot
        $arr_output_after=[];

        if (count($arr_target)>0) {
            $wo_after=WorkOrder::where('id','<>',$wo_id)->where('wo_no',$wo_no)->where('production_line_id',$line_id)->where('finish_date','>',end($arr_target)['target_date'])->orderBy('finish_date','asc')->get();
            if (count($wo_after)>0) {
                return $arr_output_after=[];
            }
        }
        ...
        if (count($arr_target)>0) {
            ...
            if ($prdln->line_refactor($sub)) {
                ...
                $mon=Monitoring::where('wo',$wo_no)->where('tanggal','>',end($arr_target)['target_date'])->where('line',$line)->get();
            }
    ...
    public function get_jamker($mon)
    {
        $jk=0;
        $b_hour=0;
    ...
    public function update_monitoring_unplanned($wo)
    {
        ...
        foreach ($mon as $value) {
            $line=$value->line;
            $unplanned=0;
            if ($prdln->line_refactor($value->branchdetail)) {
