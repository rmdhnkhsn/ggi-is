...
class AlarmController extends Controller
{
    ...
    {
        $page = 'Controll';
        return view('hr_ga/AuditBuyer/alaram/ScanAlaram', compact('page'));
    }
    ...
    public function controll(Request $request)
    {
        ...
        $bulan = date("Y-m");
            $msg="";
            if(CheckValidation::jml_controll_alarem($kode_lokasi, $bulan)) 
                $msg="Sudah melakukan pengecekan dibulan yang sama ";
            else if(CheckValidation::check_item_alarem($kode_lokasi)) 
                $msg="Item tidak ditemukan";
            if ($msg)
                return redirect()->route('hr_ga.alaram.scant')->with(['error' => $msg]);
                
        $data = ItemLokasi::where('id_item','1')->where('kode_lokasi',$kode_lokasi)->first();
    //   dd($data);
        $page = 'Controll';
        return view('hr_ga/AuditBuyer/alaram/addcontroll', compact('data','todayDate','page'));
        }
    public function store(Request $request){
        ...
        $data=AlarmBtn::where('kode_lokasi',$request->kode_lokasi)->where('tgl_periksa','>=', $tanggal_awal)->where('tgl_periksa','<=', $tanggal_akhir)->count();               
        if($data<='0'){      
            AlarmBtn::create($input);
            $Admin_hrd = (new  AuditBuyer)->Admin_hrd();
            if(($request->kondisi=='Rusak')||($request->kebersihan=='Kotor')||($request->ceklis=='Tidak')){
                foreach ($Admin_hrd as $key => $value) {
                        DB::table('notification')->insert([
                            'connection_name'=>"mysql_audit_buyer",
                            'table_name'=>"alaram_btn",
                            'alert_level'=>"DANGER",
                            'id_int'=> $request->id_alarm,
                            'nik'=> $value,
            ...
            return redirect()->route('hr_ga.alaram.scant')->with('success', 'successfully saved');
        }
        else{
            return redirect()->route('hr_ga.alaram.scant')->with('success', 'successfully saved');
        }
    ...
    public function update_perbaikan(Request $request)
        {
            ...
            AlarmBtn::whereId($id)->update($validatedData);
