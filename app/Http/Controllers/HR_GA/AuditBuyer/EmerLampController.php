...
class EmerLampController extends Controller
{
    /**
    ...
    {
        $page = 'Controll';
        return view('hr_ga/AuditBuyer/EmerLamp/ScanEmerLamp', compact('page'));
    }
    ...
    public function controll(Request $request)
    {
        ...
        // dd($bulan);
            $msg="";
            if(CheckValidation::jml_controll_emerlamp($kode_lokasi, $bulan)) 
                $msg="Sudah melakukan pengecekan dibulan yang sama ";
            else if(CheckValidation::check_item_emerlamp($kode_lokasi)) 
                $msg="Item tidak ditemukan";
            if ($msg)
                return redirect()->route('hr_ga.emerlamp.scant')->with(['error' => $msg]);
                
        $data = ItemLokasi::where('id_item','5')->where('kode_lokasi',$kode_lokasi)->first();
    //   dd($data);
        $page = 'Controll';
        return view('hr_ga/AuditBuyer/EmerLamp/addcontroll', compact('data','todayDate','page'));
        }
    public function store(Request $request){
        ...
        $data=EmerLamp::where('kode_lokasi',$request->kode_lokasi)->where('tgl_periksa','>=', $tanggal_awal)->where('tgl_periksa','<=', $tanggal_akhir)->count();               
        if($data<='0'){           
            EmerLamp::create($input);
            $Admin_hrd = (new  AuditBuyer)->Admin_hrd();
            if(($request->kondisi=='Rusak')||($request->kebersihan=='Kotor')||($request->form=='Tidak')){
                foreach ($Admin_hrd as $key => $value) {
                    DB::table('notification')->insert([
                        'connection_name'=>"mysql_audit_buyer",
                        'table_name'=>"emergency_lamp",
                        'alert_level'=>"DANGER",
                        'id_int'=> $request->id_emerlamp,
                        'nik'=> $value,
            ...
            return redirect()->route('hr_ga.emerlamp.scant')->with('success', 'successfully saved');
        }
        else{
            return redirect()->route('hr_ga.emerlamp.scant')->with('success', 'successfully saved');
        }
    ...
    public function update_perbaikan(Request $request)
    {
        ...
        EmerLamp::whereId($id)->update($validatedData);
