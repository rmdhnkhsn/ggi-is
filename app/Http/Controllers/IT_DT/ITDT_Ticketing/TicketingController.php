...
use App\Models\IT_DT\tiket\meeting_room;
...
class TicketingController extends Controller
{
    ...
    public function main(request $request)
    {
        ...
        // tiket IT supoort, RPA, DT, dan HR&GA
        ...
        $databookingD= ($bookingD)->groupBy('booking_id');
        $TiketWaitingBooking = (new TiketAll)->bookingID($bookingW);
        $TiketProcessBooking = (new TiketAll)->bookingID($bookingP);
        $TiketDoneBooking = (new TiketAll)->bookingID($databookingD);
        // endf booking
       ...
       $dataJobOrientation= JobDescription :: whereIn('bagian', ['INTERNAL CONTROL', 'DIGITAL TRANSFORMATION', 'IT','HR & GA'])
        ...
        'TiketDoneBooking','category','dataJobOrientation','dataIT','waiting_doc','process_doc','done_doc','v_mode',
    ...
    public function create(request $request)
    {
        ...
        //untuk menampilkan menu paling atas sesuai yg dipilih
        $filter_menu=[];
        ...
        $page = 'dashboard';
        $user = Auth::user("nik");
        // cek sudah pernah bikin tiket atau belum
        if( TiketUser::where('nik', $user->nik)->count()){
            ...
            $nik_manager=AppTicketAcc::where('modul','M_PCAA')->where('approvalroute_id',Auth::user()->approval_code)->where('no_slot_nik','1')->first();

            $date = date("Y-m-d ");
            $jam =date("H:i:s");
            $kategori_judul = TiketKategori::groupBy('kategori')->selectRaw('kategori')->get();
            $page = 'newticket';
            return view('it_dt.Ticketing.itdt_ticketing.create_ticket', compact( 'data','user','date','jam','kategori_judul', 'page','filter_menu','jenis_pencairan','data_rek','nik_manager'));
        }
        // apabila belum pernah bikin tiket lengkapi terlebih dahulu ext dan ip
        else{
            $data = Auth::user("nik");
            $page = 'newticket';
    ...
    public function store(Request $request)
    {
        ...
        // store data tiket IT, IT & DT, HR & GA
        $data=Tiket::create([
    ...
    public function adminDT()
    {
        ...
        $it_support= (new TiketAll)->IT_support($data_it);
           ...
           // olah data tiket IT DT semua status 
           ...
           // menampilkan data tiket IT DT semua status sesuai bagian supoort(JDE, HRIS, GCC) 
    ...
    public function adminBooking(Request $request)
    {
        if($request->branch!=null || $request->branch!=""){
            ...
            $branch=branch::where('branchdetail', auth()->user()->branchdetail)->first();
        }
        $page = 'admin_rbo';
        $waktu = date('H.i.s');
        $tgl_akhir = date('Y-m-d');
        $master_support=TiketIT::all();
        $dataBranch=branch::wherein('id',[1,2,3])->get();
        $bookingW=TiketBookingDetail::where('tanggal_booking','>',$tgl_akhir)
                                ->where('status_booking','=','WAITING')
                                ->where('branchdetail', $branch->branchdetail)->get()->groupBy('booking_id');
        $bookingP=TiketBookingDetail::where('tanggal_booking',$tgl_akhir)
                                ->where('waktu_finish','>=',$waktu)->where('status_booking','=','WAITING')
                                ->where('branchdetail', $branch->branchdetail)->get()->groupBy('booking_id');    
        $bookingDone=TiketBookingDetail::where('tanggal_booking',$tgl_akhir)->where('waktu_finish','<',$waktu)
                                ->where('status_booking','=','WAITING')
                                ->where('branchdetail', $branch->branchdetail)->get()->groupBy('booking_id');  
        ...
        $TiketWaitingBooking = (new TiketAll)->bookingID($bookingW);
        $TiketProcessBooking = (new TiketAll)->bookingID($bookingP);
        $TiketCancelBooking = (new TiketAll)->bookingID($bookingC);
        $TiketDoneBooking = (new TiketAll)->bookingID($bookingDone);
        
        $data_admin=tiketIT::where('bagian','RECEPTIONIST')->where('branch',$branch->kode_branch)
            ->where('branchdetail', $branch->branchdetail)->get();
        ...
        ,'waiting','done','proggres','cancel','TiketWaitingBooking','TiketProcessBooking','TiketDoneBooking','TiketCancelBooking','dataBranch','branch'));
    }
    public function createBooking(Request $request)
    {
            ...
            $page = 'newticket';
            $dataBranch=branch::wherein('id',[1,2,3])->get();
            return view('it_dt.Ticketing.itdt_ticketing.create_booking', compact( 'data','user','tgl_sekarang','page','tanggal7','dataBranch'));
    }
    public function monitoringBooking(Request $request)
    {
        ...
        $dataBranch=branch::wherein('id',[1,2,3])->get();
        $branch=$request->branch;
        return view('it_dt.Ticketing.itdt_ticketing.booking_monitoring', compact( 'data','user','date','page','dataBranch','branch'));
    }
    public function showBookingTicket(Request $request) 
    {
        $branch=Branch::findOrFail($request->branchSelected);
        $date =date('Y-m-d', strtotime($request->bookingDate));
        $ruang_meeting=meeting_room::where('branchdetail',$branch->branchdetail)->get();
        foreach ($ruang_meeting as $key => $value) {
            $booked_ticket = TiketBookingDetail::where("tanggal_booking", $date)
                        ->where("ruang_meeting", $value->room)
                        ->where('branchdetail',$branch->branchdetail)
                        ->whereNull("is_cancel")
                        ->selectRaw("waktu_id, ruang_meeting,nama,deskripsi,is_cancel")
                        ->groupBy(["waktu_id", "ruang_meeting","nama","deskripsi","is_cancel"]);
            $waktu =TiketWaktu::leftJoinSub($booked_ticket, 'booked_ticket',
                        function($join){
    ...
    public function verifyDuplicateBookingTicket($tanggal_booking, $id_waktu, $ruang_meeting) {
        $response = (object)[
            "is_error" => false,
            "message" => "success"
        ];
        $booked_ticket = 
            ...
            ->where("ruang_meeting", $ruang_meeting->room)
            ->where("branch", $ruang_meeting->kode_branch)
            ->where("branchdetail", $ruang_meeting->branchdetail)
            ->wherein("waktu_id", $id_waktu)
            ->where("is_cancel",null)
            ->count();
        if ($booked_ticket>0) {
            $response->is_error = true;
            $response->message = "Maaf Ruang dengan waktu sudah penuh!";
        }
        return $response;
    }
    public function storeBooking(Request $request)
    {
        try{
            ...
            $room=meeting_room::findOrFail($request->room_meeting);
            $verify_duplicate_booking_ticket = $this->verifyDuplicateBookingTicket($request->tanggal_booking, $id_waktu, $room );
            if ($verify_duplicate_booking_ticket->is_error) {
                ...
                foreach ($id_waktu as $key => $value) {
                    $str_waktu=TiketWaktu::where('id',$value)->first();
                        $data_boking=[
    ...
    public function prosesBookingCancel(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'The room has been successfully canceled');
    }
    ...
    public function update_tiketUser_doc(Request $request)
    {
        ...
        //update dan hapus data file sebelumnya
