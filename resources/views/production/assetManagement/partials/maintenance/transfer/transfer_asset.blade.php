...
<section class="content index">
    <div class="container-fluid">
        ...
        <form  action="{{route('asset.maintenance.updateTransferAssets')}}" method="post" enctype="multipart/form-data">
            @csrf 
            <div class="container">
                <div class="contents mx-auto mt-mins">
                    <div class="title-30W text-center mb-4">Transfer Asset</div>
                    <div class="card p-3">
                            ...
                            <div class="widthHalf">
                                ...
                                <div class="input-group mt-1 mb-3">
                                    ...
                                    <select class="form-control borderInput pointer select2bs4" id="" name="location" required>
                                        ...
                                        <option name="{{ $value['id'] }}">{{ $value['tipe'] }}/{{ $value['nama'] }}</option>    
                                        @endforeach
                                    </select>
                                </div>
                            </div>
    ...
    if (decodedText !== lastResult) {
        ...
        success: function(resp){
            if(!resp.id) {
                ...
                $("#varian_"+rowNumber).val(resp.coOrigin)
                $("#supplier_"+rowNumber).val(resp.supplier)
                $('.modal').modal('toggle');
            }}
        })
        }
...
<script src="{{asset('common/js/instascan.js')}}"></script>
...
<script>
  $('.saveTransfer').on('click', function (event) {
    ...
    const url = $(this).attr('href');
      swal({
        title: "Are You Sure..?",
        text: "For Saving transfer assets !",
        type: "warning",
    ...
    function(isConfirm){
        if (isConfirm) {
         location.reload();        // submitting the form when user press yes
        } else {
        swal("Batal", "Transfer Assets failed to save", "error");
        }
    }); 
  });
...
<script>
  ...
  $("#addAssets").click(function () {
    var location = ``
       TotalBaris = parseInt($("#TotalBaris").val());                 
    var html = '';
        ...
        html +='</div>';
        html +='<input type="text" class="form-control borderInput2" name="serialno[]" id="serialno_'+TotalBaris+'" placeholder="Input serial number"><input type="hidden" name="id_machine[]" id="id_machine_'+TotalBaris+'"><input type="hidden" name="id[]" id="id_'+TotalBaris+'">';
        html +='<div class="input-group-prepend">';
        ...
        html +='</div>';
        html +='<input type="text" class="form-control borderInput" name="jenis[]" id="jenis_'+TotalBaris+'" placeholder="Input jenis Asset" readonly>';
        html +='</div>';
        html +='</div>';
        html +='<div class="widthHalf">';
        html +='<div class="input-group">';
        html +='<div class="input-group-prepend">';
        html +='<span class="inputGroupBlue" style="width:45px"><i class="fs-20 fas fa-list-ol"></i></span>';
        html +='</div>';
        html +='<input type="text" class="form-control borderInput" name="kondisi[]" id="kondisi_'+TotalBaris+'" style="border-radius:0 10px 10px 0" placeholder="Input kondisi" readonly>';
        ...
        html +='<input type="hidden" class="form-control borderInput" name="company[]" id="company_'+TotalBaris+'" placeholder="Input Origin" readonly>';
        html +='<input type="hidden" class="form-control borderInput" name="fromLokasi[]" id="fromLokasi_'+TotalBaris+'" placeholder="Input Origin" readonly>';
        html +='<input type="hidden" class="form-control borderInput" name="kondisi[]" id="kondisi_'+TotalBaris+'" placeholder="Input Origin" readonly>';
