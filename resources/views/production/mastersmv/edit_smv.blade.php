            ...
            <div class="row">
              <div class="col-12 justify-start gap-8">
                <div class="title-20 text-left">Data Process</div>
                 <button type="button" class="icon-circle-blue" data-toggle="modal" data-target="#detailDataProses2{{ $data2['id'] }}"><i class="fas fa-info"></i></button>
                    @include('production.mastersmv.partials.database_smv.detailsDataproses2')
              </div>
            </div>
            <form  action="{{route('mastersmv.updateSmvChild',$data->id)}}" method="post" enctype="multipart/form-data">
            @csrf
                            ...
                            <table id="DTtable" class="table tbl-content-center no-wrap">
                                ...
                                <tbody>
                                    @php
                                        $i = 1;
                                    @endphp
                                    @foreach ($masterDatabase as $key =>$value)
                                   
                                    <tr>
                                         <input type="hidden" name='jumlahId[]' value={{ $value['smv_id'] }}>
                                        {{-- <input type="hidden" class="allowance" value="15"> --}}
                                        {{-- <input type="hidden" class="target"  id="targets" name="targets" value="{{ $value['targets'] }}"> --}}
                                        <input type="hidden" class="productivity" value="5">
                                        <td><a href="{{ route('mastersmv.deleteSmvChild',$value->smv_id) }}" class="btn-delete-row2 container-tbl-btn deleteFile"><i class="far fa-times-circle"></i></a></td>
                                        <td><input type="text" class="form-control borderInput container-tbl-btn nama_proses2" name="nama_proses[]" value="{{ $value->nama_proses }}"></td>
                                        <td>
                                            <select class="form-control borderInput pointer container-tbl-btn" id="cat" name="cat[]" style="width:160px">
                                                <option selected> {{ $value->cat }}</option>
                                                <option name="cat[]" value="M">M</option>
                                                <option name="cat[]" value="Iron">Iron</option>
                                                <option name="cat[]" value="Helper">Helper</option>
                                                <option name="cat[]" value="B">B</option>
                                                <option name="cat[]" value="QC">QC</option>
                                            </select>
                                          </td>
                                        <td><input type="text" class="form-control borderInput container-tbl-btn cycle_time cycle_timerecord{{ $i }}" data-record_id="{{ $i }}" name="cycle_time[]" value="{{ $value->cycle_time }}" onchange="hitung(this,{{ $i }})" id="cycle_time{{ $i }}"></td>
                                         <input type="hidden" class="allowance" name="allowance" value="{{ $data2['allowance'] }}">
                                        <input type="hidden" class="manpowerHeader" name="manpower" value="{{ $data2['manpower'] }}">
                                        <td><input type="text" class="form-control borderInput container-tbl-btn smv_minute smv_minuterecord{{ $i }}" data-record_id="{{ $i }}" name="smv_minute[]" value="{{ $value->smv_minute }}" readonly></td>
                                        <td><input type="text" class="form-control borderInput container-tbl-btn pardOncapacity pardOncapacityrecord1" data-record_id="1" name="output_pj[]" value="{{ round($value->output_pj),3 }}" readonly></td>
                                        <td><input type="text" class="form-control borderInput container-tbl-btn actualMP actualMPrecord{{ $i }}" data-record_id="`" name="actual_mp[]" value="{{ $value->actual_mp }}" readonly></td>
                                        <td><input type="text" class="form-control borderInput container-tbl-btn manpower manpowerrecord{{ $i }}" data-record_id="`" name="manpower_need[]" value="{{ $value->manpower_need }}" readonly></td>
                                        <td><input type="text" class="form-control borderInput container-tbl-btn workingTime workingTimerecord{{ $i }}" data-record_id="`" name="working_time[]" value="{{ $value->working_time }}" readonly></td>
                                        <td><input type="text" class="form-control borderInput container-tbl-btn workingBalance workingBalancerecord{{ $i }}" data-record_id="`" name="working_balance[]" value="{{ $value->working_balance }}" readonly></td>
                                        <td><input type="text" class="form-control borderInput container-tbl-btn actualUnit actualUnitrecord{{ $i }}" data-record_id="`"  name="actual_unit[]" value="{{ $value->actual_unit }}" readonly></td>
                                        <td><input type="text" class="form-control borderInput container-tbl-btn attachment1" name="attachment1[]" value="{{ $value->attachment1 }}"></td>
                                        <td><input type="text" class="form-control borderInput container-tbl-btn mesin" name="mesin[]" value="{{ $value->mesin }}" style="width:170px"></td>
                                    </tr>
                                    <script>
                                        $("#targets").val("{{ $value['targets'] }}");
                                        $("#JmlRow").val("{{ $i }}");
                                    </script>
                                    @php
                                        $i++;
...
<script type="text/javascript">
    ...
    function sumTotalTarget() {
        ...
        // console.log(manpowerHeader)
        // console.log(targetperhourse)
        // console.log("total_targe:"+total_target)
        // console.log("total_targe:"+TotalTarget)
        target = TotalTarget.toFixed(0); 
        // return TotalTarget.toFixed(0)
...
<script>
  $('.deleteFile').on('click', function (event) {
    ...
    swal({
        title: 'Are you sure?',
        text: 'This record Database Smv and Data Process will be permanantly deleted!',
        icon: 'warning',
        buttons: ["Cancel", "Yes"],
    }).then(function(value) {
        if (value) {
