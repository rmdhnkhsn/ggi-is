          ...
          <div class="col-12">
            <div class="row" style="padding-bottom:30px;">
                <a href="{{route('prd.grafik')}}" class="col-lg-6 col-6">
                    <div>
                        <div class="label_abu"> CHART PARAMETER WAKTU SYSTEM SIGNAL TOWER</div>
                        <div class="container btn_abu"></div>
                    </div>
                </a>
                <a href="{{route('prd.grafikBatang')}}" class="col-lg-6 col-6">
                    <div>
                        <div class="label_ijo"> CHART PARAMETER QUANTITY SYSTEM SIGNAL TOWER</div>
    ...
    fetch("{{ route('prd.grafik.getData') }}")
      ...
      .then(response => {
        ...
        console.log('sumAllDataPerLineAndDate', sumAllDataPerLineAndDate);
          ...
          var dateResult = []
          var totalRequest = []
          var avgtotalRequestPerDay = []
          var avgtotalActualPerDay = []
        ...
        for (let i = 0; i < avgAllDataPerLineAndDate.length; ++i) {
            ...
            var num = Number(avgAllDataPerLineAndDate[i].avgtotalActualPerDay);
            var roundedString = num.toFixed(2);
            var rounded = Number(roundedString);

            avgtotalActualPerDay.push(rounded);        
              
            var num = Number(avgAllDataPerLineAndDate[i].avgtotalRequestPerDay);
            var roundedString = num.toFixed(2);
            var rounded = Number(roundedString);
            avgtotalRequestPerDay.push(rounded);
        ...
        var options = {
          series: [{
            name: 'Qty Request Day (Pc)',
            type: 'bar',
            data: avgtotalRequestPerDay
          }, {
            name: 'Qty Actual Day (Pc)',
            type: 'bar',
            data: avgtotalActualPerDay
          },],
          chart: {
            width: "90%",
            height: 650,
        ...
        title: {
            text: 'Chart Parameter Quantity System Signal Tower',
            align: 'center',
            margin: 90,
            offsetX: 0,
            offsetY: 0,
