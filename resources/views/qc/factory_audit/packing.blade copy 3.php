...
<script>
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (type != "article") {
            ...
            for (let i = 0; i < data.wo_information.length; i++) {
            article_option += `<option data-id_wo="`+data.wo_information[i].id_wo+`" data-is_empty_article="`+data.wo_information[i].is_empty_article+`" data-buyer="`+data.wo_information[i].buyer+`" value="`+data.wo_information[i].article+`">`+data.wo_information[i].id_wo+`|`+data.wo_information[i].article+`</option>`
            }
    ...
    function showAllWoInformation() {
        $.ajax({
            ...
            success: function (data) {
                article_option = `<option class="form-control selected disabled> </option>`
                for (let i = 0; i < data.wo_information.length; i++) {
                    article_option += `<option data-id_wo="`+data.wo_information[i].id_wo+`" value="`+data.wo_information[i].id_wo+`|`+data.wo_information[i].article+`">`+data.wo_information[i].article+`</option>`
                }
    ...
    $('body').on('change', '.load-article', function () {
        var values = $(this).val()
        var arr_value = values.split('|')
        var id_wo = arr_value[1]
        var article = arr_value[0]
        console.log([arr_value, id_wo, article])
