                            ...
                            <div class="cards-scroll scrollX" id="scroll-bar">
                                <table id="DTtable" class="table tbl-content">
                                    ...
                                    <tbody>
                                        ...
                                        <?php
                                            $tanggal = date('Y-m-d', strtotime($value->tanggal_trans));
                                        ...
                                        <tr>
                                            <td>{{$value->id}}</td>
                                            <td>{{$tanggal}}</td>
                                            <td>{{$value->jenis_bc}}</td>
                                            <td>{{$value->bpb}}</td>

                                            <td>{{$value->from_branchdetail}}</td>
                                            <td>{{$value->addressbook($value->to_branchdetail)}}</td> 

                                            <td>{{$value->list_subkon()}}</td>
                                            <td>{{$value->surat_jalan}}</td>
                                            <td>{{$value->dokumen_note}}</td>

                                            <!-- Kondisi Waktu  -->
                                            @if($value->status_barang == 'Mekanik')
                                            <td>{{date('H:i:s', strtotime($value->created_at))}}</td> 
                                            @elseif($value->status_barang == 'Gudang')
                                            <td>{{date('H:i:s', strtotime($value->created_at))}}</td> 
                                            ...
                                            @if($value->status_barang == 'Mekanik')
                                            <td><span class="text-merah">Mekanik</span></td> 
                                            @elseif($value->status_barang == 'Gudang')
                                            ...
                                            <td>
                                                <div class="justify-end gap-2">
                                                    @if($value->status_barang == "Ekspedisi")
                                                        ...
                                                        @endif
                                                    @else
                                                        <!-- Tombol kirim untuk mekanik  -->
                                                        @if(($value->status_barang == 'Mekanik' && auth()->user()->departemensubsub == 'MEKANIK'))
                                                        <a href="{{route('in-out.next_step', $value->id)}}"  class="btn-blue-md">Kirim <i class="ml-2 fs-18 fas fa-paper-plane"></i></a>
                                                        <!-- Tombol kirim untuk warehouse  -->
                                                        @elseif(($value->status_barang == 'Gudang' && auth()->user()->departemensubsub == 'WAREHOUSE') || ($value->status_barang == 'Gudang' && auth()->user()->jabatan == 'KABAG'))
                                                        <a href="{{route('in-out.next_step', $value->id)}}"  class="btn-blue-md">Kirim <i class="ml-2 fs-18 fas fa-paper-plane"></i></a>
                                                        <!-- Tombol kirim untuk dokumen  -->
                                                        @elseif($value->status_barang == 'Dokumen' && auth()->user()->departemensubsub == 'EXIM')
                                                        <a href="{{route('in-out.next_step', $value->id)}}"  class="btn-green-md">Finish <i class="ml-2 fs-18 fas fa-paper-plane"></i></a>
                                                    ...
                                                    <div class="dropdown-menu">
                                                        ...
                                                        <!-- Button Edit  -->
                                                        <button class="dropdown-item voidBpb" data-toggle="modal" data-target="#voidBpb{{$value->id}}" style="color:#FF0000 !important"><i class="mr-2 fs-18 fas fa-trash"></i>Void BPB</button>
                                                       
                                                        @if($value->status_barang == "Gudang" && auth()->user()->departemensubsub == 'WAREHOUSE' || auth()->user()->jabatan == 'KABAG')
                                                        ...
                                                        @endif
                                                        <!-- Button Reverse  -->
                                                        @if($value->status_barang == 'Dokumen' && auth()->user()->departemensubsub == 'EXIM')
                                                        <button class="dropdown-item notedelay" data-toggle="modal" data-target="#noteDokumen{{$value->id}}"><i class="mr-2 fs-18 fas fa-info-circle"></i>Delay Note</button>
                                                        <a href="{{route('in-out.reverse_step', $value->id)}}" class="dropdown-item" style="color:#68717A"><i class="mr-2 fs-18 fas fa-backward"></i>Reverse</a>
                                                        @elseif($value->status_barang == 'Ekspedisi')
                                                            @if(auth()->user()->departemensubsub == 'EXPEDISI' || auth()->user()->departemensubsub == 'EKSPEDISI')
                                                                <a href="{{route('in-out.reverse_step', $value->id)}}" class="dropdown-item" style="color:#68717A"><i class="mr-2 fs-18 fas fa-backward"></i>Reverse</a>
                                                            @endif
                                                        @elseif($value->status_barang == 'Finish' && auth()->user()->departemensubsub == 'EXIM')
                                                        <a href="{{route('in-out.reverse_step', $value->id)}}" class="dropdown-item" style="color:#68717A"><i class="mr-2 fs-18 fas fa-backward"></i>Reverse</a>
                                                        @endif
                                                        <!-- Button Delete  -->
...
@endif
<script>
    $("#filter_in_hand").val("{{$filter_in_hand}}").change();
    $("#filter_from_branchdetail").val("{{$filter_from_branchdetail}}").change();
    $("#filter_tanggal_trans").val("{{$filter_tanggal_trans}}").change();
