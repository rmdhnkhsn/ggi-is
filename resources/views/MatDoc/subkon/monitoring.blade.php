      ...
      <a href="{{ route('subkon.monitoring',$no_kontrak)}}" class="rc-col-2">
        <div class="cards bg-card-active h-95 p-3">
          <div class="row">
            ...
            <div class="col-12">
              <div class="rc-desc-active">MONITORING</div>
            </div>
          </div>
        </div>
      </a>
          ...
          success:function(res){               
            if(res){
              // console.log()
                ...
                $("#qtyPj").val(res.kebutuhan);
                document.getElementById('qtyMaterial').setAttribute('max', parseInt(res.kebutuhan))
            }else{
            }
          }
        });
...
<script>
  ...
  document.getElementById('buttonpengeluaran').addEventListener('click', function(e) {
    if(e.target.parentNode.parentNode.getElementsByTagName('input')[1].value != ''  && e.target.parentNode.parentNode.getElementsByTagName('input')[2].value != '') {
      ...
      alert('harap isi nomor BPB dan No Penyerahan Terlebih dahulu.')
    }
  })
