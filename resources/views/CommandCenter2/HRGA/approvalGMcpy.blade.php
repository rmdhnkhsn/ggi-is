    ...
    let tolongcektotal = ()=> {
      ...
      dataBranch.map((branch)=> {
        if (branch === document.getElementById(branch).id) {
          ...
          console.log('data branch tidak ditemukan')
        }
      })
    }
  ...
  $(document).ready( function () {
    ...
    let tolongcektotal = ()=> {
      let total = 0;
        Array.from(inicheckbox).forEach(function(element) {
          if(element.checked) {
            let tmp = parseFloat(element.parentNode.parentNode.getElementsByClassName('amountpengajuan')[0].value)
            total += tmp;

            console.log(element.parentNode.parentNode.getElementsByClassName('branchdetail')[0].value);
