...
<!-- <section class="content-header f-poppins">
      <div class="card-navigate">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item title-navigate-active" aria-current="page">Quality Control</li>
            <li class="navigate-active ml-qc"></li>
          </ol>
        </nav>
      </div>
    </section> -->
    ...
    <section class="content f-poppins">
          ...
          <div class="col-12">
            ...
            <div class="btn-group">
              <button type="button" class="btn btn-dropdown dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                <span class="allfac-ddown">{{$dataBranch->nama_branch}}</span>
              </button>
      ...
      <div class="row">
        @foreach($dataValue as $qc)
        @if($qc['nama'] == 'REWORK LINE')
        <a href="{{ route('cc2.rework', $dataBranch->id) }}" class="col-sm-12 col-md-6 col-xl-3">
        @else
            ...
            <div class="row">
              <div class="col-12">
                ...
                <div class="container-knob-itdt">
                  <input class="dial" data-displayPrevious=true data-fgColor="#FB5B5B" data-skin="tron" data-width="160" data-thickness=".19" data-height="160" value="{{$qc['nilai']}}" disabled>
                  <div class="knob-label" id="labelknob-critical">CRITICAL</div>
                </div>
                @endif
                @if($qc['issues'] == 0)
                <div class="container-knob-itdt">
                  <input class="dial" data-displayPrevious=true data-fgColor="#0cae63" data-skin="tron" data-width="160" data-thickness=".19" data-height="160" value="{{$qc['nilai']}}" disabled>
                  <div class="knob-label" id="labelknob-good">GOOD</div>
                </div>
                @endif
                <div class="allfac-issue2">
                  <span class="branch">{{$qc['nama']}}</span>
                  <span class="tot-issue">{{$qc['issues']}} Issues</span>
                </div>
                </center>
              </div>
...
@push("scripts")
@if(Session::has('rework'))
  <script>
    toastr.error("{!!Session::get('rework')!!}");
  </script>
@endif

<script>
  var donutChart1 = {
    series: <?php echo json_encode($dataknob); ?>,
    chart: {
      type: 'donut',
    },
...
<script>
    ...
    colors: ['#FB5B5B','#FC6F6F','#FC8282','#FD9696','#FCA9A9','#FDBDBD','#FED1D1'],
