            ...
            <div class="joblist-date">
                ...
                <div class="margin-date">
                    <div class="input-group date" id="Date6" data-target-input="nearest">
                        <div class="input-group-append datepiker" data-target="#Date6" data-toggle="datetimepicker">
                            <div class="custom-calendar" ><i class="fas fa-calendar-alt mr-2"></i> <span class="fs-14">Date</span><i class="ml-2 fas fa-caret-down"></i>
                            </div>
                        </div>
                        <input type="text" class="form-control datetimepicker-input border-input-grey" data-target="#Date6" placeholder="Select Date" style="width:125px"/>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="row">
      <div class="col-12 pb-5">
        <div class="cards-scroll scrollX" id="scroll-bar">
            ...
            <table id="DTtable_sticky6" class="table table-content tr-hover no-wrap py-2">
                <thead>
                    <tr>
                ...
                <tbody>
                    @foreach ($dataSampleDev as $key => $value)
                        <tr>
                            ...
                            <td>
                                <form action="{{route ('sample-updateDepartementTrecking') }}" method="post" enctype="multipart/form-data">
                                    <a href="" class="tbl-btn-dailyUpdate" data-toggle="modal" data-target="#dailyUpdateDev{{ $value['id'] }}"><i class="fs-20 fas fa-calendar-alt"></i></a>
                                    <a href="{{ route('sample.detailDone',$value['id'])}}" class="tbl-btn-info"><i class="fas fa-info"></i></a>
                                    <input type="hidden" id="id" name="id" value="{{ $value['id'] }}">
                                    <input type="hidden" id="departement_trecking" name="departement_trecking" value="CUTTING">
                                ...
                                @include('sample_room.status_sample.partials.sample_request.modal.assign_dev')
                            </td>
                        </tr>
                    @endforeach
                </tbody>
