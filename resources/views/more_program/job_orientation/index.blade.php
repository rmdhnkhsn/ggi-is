    ...
    <div class="row JobOri">
            ...
            <div class="cards" style="padding:20px 28px 16px 28px">
                <div class="cards-scroll pb-2 scrollX" id="scroll-bar">
                    <ul class="nav nav-pills flex" role="tablist" style="gap:.8rem !important;flex-wrap: nowrap;">
                        @include('more_program.job_orientation.partials.nav-tabs')
                    </ul>
                </div>
            </div>
        </div>
        <div class="col-12">
            <div class="tab-content">
                ...
                <div class="tab-pane fade" id="{{str_replace([' & ', ' '], '_', $value['departemen'])}}" data-dept="{{$value['departemen']}}" role="tabpanel">
                  @include('more_program.job_orientation.partials.tab-index')
                </div>
                @endforeach
            </div>
        </div>
...
<script>
  ...
  if (title != null) {
    ...
    function show (data) {
      ...
      // console.log(tab_bagian);
      $('.cobaanwe').hide();
      $('.inti_bagian').hide();
      // console.log(inicoba);
      // const tabpane = document.getAttribute('data-dept');

      data.forEach(function(e) {
        Array.from(tabpane).forEach(function (element) {
          Array.from(inicoba).forEach(function (teuing) {
            if (element.getAttribute('coba_atribut') == e.departemen) {
        ...
        Array.from(tab_bagian).forEach(function (element_nya) {
          Array.from(inti_bagian).forEach(function (teuing_atuh) {
            if (element_nya.getAttribute('departemen') == ehh.departemen) {
              if (element_nya.getAttribute('bagian') == ehh.bagian) {
