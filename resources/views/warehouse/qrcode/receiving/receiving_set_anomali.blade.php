...
<section class="content">
  <div class="container-fluid">
      <table id="DTtable3" class="table tbl-content-left no-wrap">
        ...
        <tbody>
          <form action="{{route('store-anomali-receiving') }}" method="post" enctype="multipart/form-data">
            @csrf  
            @foreach ($data as $k=>$v)
              <tr>
                <!-- <td>{{$k+1}}</td> -->
                ...
                <td>{{$v->trans_uom}}</td>
                <td>
                <input type="hidden" name="warehouse_delivery_id" value="{{ $v->warehouse_delivery_id }}">

                  <input type="hidden" name="id[]" value="{{ $v->id }}">
                  <input type="text" class="form-control border-input" name="remark[]" value="{{ $v->remark }}" placeholder="masukan remark...">
                </td>
              </tr>
            @endforeach
