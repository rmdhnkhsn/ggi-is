...
<section class="content">
                ...
                <div class="cards-18" style="padding:1.5rem">
                    ...
                    <div class="cards-scroll scrollX" id="scroll-bar">
                        <?php
                            ...
                            $groupBySize = collect($value)->groupby('size_packing_list');
                        ?>
    ...
    <?php 
        $cobaSaja  = [];
        foreach ($dataTabel as $key => $value) {
            $groupSize = collect($value)->groupBy('size_packing_list');
            foreach ($groupSize as $key2 => $value2) {
                $size_jumlah = collect($value2)->sum('jumlah_size');
                $SizeNya = collect($value2)->first();
                $cobaSaja[] = [
                    'qty_carton' => $SizeNya['qty_carton'],
                    'jumlah_size' => $size_jumlah * $SizeNya['qty_carton'],
                    'nw' => $SizeNya['nw'],
                    'gw' => $SizeNya['gw'],
        ...
        <div class="row">
            <div class="rc-col-2">
                <div class="sb-card p-3">
                    ...
                    <div class="txt2">{{collect($resume)->sum('qty_carton')}}</div>
                </div>
            </div>
            <div class="rc-col-2">
                <div class="sb-card p-3">
                    <div class="txt1">Total Pcs</div>
                    <div class="txt2">{{collect($resume)->sum('jumlah_size')}}</div>
                </div>
            </div>
            <div class="rc-col-2">
                <div class="sb-card p-3">
                    <div class="txt1">NW</div>
                    <div class="txt2">{{collect($resume)->sum('nw')}}</div>
                </div>
            </div>
            <div class="rc-col-2">
                <div class="sb-card p-3">
                    <div class="txt1">GW</div>
                    <div class="txt2">{{collect($resume)->sum('gw')}}</div>
                </div>
            </div>
        </div>
    </div>
