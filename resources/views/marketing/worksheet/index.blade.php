      ...
      <div class="col-12">
        <div class="cards p-4">
          ...
          <div class="tab-content card-block">
            <!-- Rekap Order  -->
            <input type="hidden" id="po_list_bulan" value="{{$polist_id}}">
                  ...
                  <div class="filterSMV">
                      <div class="input-group justify-center">
                          ...
                          <div class="input-group date" id="report_date" data-target-input="nearest">
                            ...
                            data-target="#report_date" placeholder="Select Month" style="border-radius:0 10px 10px 0"/>
              ...
              <div class="row">
                <div class="col-12 pb-5">
                  ...
                  <div class="cards-scroll scrollX" id="scroll-bar">
                    <table id="POlist" class="table tbl-content-left no-wrap" style="width:100%" >
                      <thead>
                        <tr class="bg-thead">
                          <th>PO Number</th>
                          <th>Style</th>
            ...
            <!-- Worksheet  -->
            <input type="hidden" id="worksheet_list_bulan" value="{{$worksheet_id}}">
            <div class="tab-pane" id="worksheet" role="tabpanel">
              <div class="row">
                <div class="col-12 justify-sb2">
                  ...
                  <div class="filterSMV">
                    <div class="input-group justify-center">
                      <!-- <div class="sub-title-14 mr-2">Show : </div> -->
                      <div class="input-group date" id="worksheet_report_date" data-target-input="nearest">
                        <div class="input-group-append datepiker" data-target="#worksheet_report_date" data-toggle="datetimepicker">
                          <div class="inputGroupBlue"><i class="fas fa-calendar-alt mr-2 fs-18"></i><i class="fas fa-caret-down"></i></div>
                        </div>
                        <input type="text" class="form-control datetimepicker-input borderInput w-130"
                        data-target="#worksheet_report_date" placeholder="Select Month" style="border-radius:0 10px 10px 0"/>
                        <input type="hidden" id="worksheet_nilai_filter" type="text" value="" />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="row">
                <div class="col-12 pb-5">
                  <button type="button" id="btnSearch" class="iconSearch"><i class="fas fa-search"></i></button>
                  <div class="cards-scroll scrollX" id="scroll-bar">
                    <table id="Worksheet" class="table tbl-content-left no-wrap" style="width:100%">
                      <thead>
                        <tr class="bg-thead">
                          <th>PO Number</th>
                          <th>Style</th>
...
<script type="text/javascript">
  $(function () {
    var id = $('#worksheet_list_bulan').val();
    var url = "{{ route('worksheet.list',[':id']) }}";
    url=url.replace(':id',id);
...
<script>
  jQuery(document).ready(function($) {
    $('#worksheet_report_date').datetimepicker({
      format: 'Y-MM',
      showButtonPanel: true
    })
    
    var filter_count = 0;
    var filter=$('#worksheet_nilai_filter').val();
    $("#worksheet_report_date").on("change.datetimepicker", ({date}) => {
        if (filter_count > 0) {
            var worksheet_month = $("#worksheet_report_date").find("input").val();
            console.log(worksheet_month)
            var url = "{{ route('worksheet.list',[':id']) }}";
            url=url.replace(':id',worksheet_month);
            window.location.href = url;
          }
          filter_count++
    })
